...
class TestAnnCoordinates_GENCODE(unittest.TestCase):
...
class TestAnnCoordinates_REFSEQ(unittest.TestCase):
    ...
    default_args = {
        'mysql_config': None,
        'ann_source': 'RefSeq',
        'ann_refseq_files': None,  # wait to assign
        'use_cache': True,
        'logger_level': 50,  # logging.CRITICAL
    }
    ...
    def setUpClass(cls):
        ...
        try:
            assert isinstance(cls.REFSEQ_FILES, (str, list, set))
        except Exception:
            # Skip all test
            cls.skipTest(cls, "Invalid RefSeq files: {}".format(cls.REFSEQ_FILES))
        else:
            cls.default_args['ann_refseq_files'] = cls.REFSEQ_FILES
    ...
    def _test_ann_alk_(self, args):
        ...
        # ALK chr2:29,415,640-30,144,477(GRCh37/hg19) from GeneCards
        # ALK chr2:29,415,640-30,144,432(GRCh37/hg19) from GENCODE/HAVANA
        # ALK chr2:29,415,639-30,144,452(GRCh37/hg19) from RefSeq NM_004304.5
        ...
        ac.query('2:30,143,526')
        gene_info = ac.gene_info
        self.assertEqual(gene_info['id'], 'ALK')
        self.assertEqual(gene_info['name'], 'ALK')
        self.assertEqual(gene_info['type'], 'protein_coding')
        self.assertEqual(gene_info['strand'], '-')
        rank_info = ac.rank_info
        self.assertEqual(rank_info['trans_id'], 'NM_004304.5')
        self.assertEqual(rank_info['trans_start'], 29415639)
        self.assertEqual(rank_info['trans_end'], 30144452)
        self.assertEqual(rank_info['trans_strand'], '-')
        self.assertEqual(rank_info['exon_total'], 29)
        self.assertEqual(rank_info['cds_min'], 1)
        self.assertEqual(rank_info['cds_max'], 29)
        ...
        self.assertEqual(rank_info['exon_num'], 1)
        # Exon20
        bp = np.random.choice(range(29446207, 29446394 + 1))
        ac.query("2:{}".format(bp))
        gene_info = ac.gene_info
        self.assertEqual(gene_info['id'], 'ALK')
        self.assertEqual(gene_info['name'], 'ALK')
        ...
        self.assertEqual(rank_info['exon_num'], 20)
        # Intron10
        bp = np.random.choice(range(29498093 + 1, 29498267))
        ac.query("2:{}".format(bp))
        gene_info = ac.gene_info
        self.assertEqual(gene_info['id'], 'ALK')
        self.assertEqual(gene_info['name'], 'ALK')
    ...
    def test_05_ann_intergenic_igh_gene(self):
        ...
        ac = AnnCoordinates(**args)
        # IGHJ6 chr14:106329405-106329470
        ac.query('chr14:106329469')
        self.assertEqual(len(ac.gene_hits), 1)
        gene_hit = ac.gene_hits[0]
        # self.assertIsNone(gene_hit['id'])
        self.assertEqual(gene_hit['name'], 'IGHJ6')
        self.assertEqual(gene_hit['type'], 'protein_coding')
        # self.assertIsNone(gene_hit['source'])
        self.assertEqual(gene_hit['strand'], '-')
        rank_info = ac.rank_info
        self.assertEqual(rank_info['trans_strand'], '-')
        # IGHJ3P chr14:106329624-106329675
        # intergenic(IGHJ6,IGHJ3P)
        ac.query('chr14:106329500')
        self.assertEqual(len(ac.gene_hits), 1)
        gene_hit = ac.gene_hits[0]
        self.assertIsNone(gene_hit['id'])
        self.assertEqual(gene_hit['name'], 'IGHJ6')
    ...
    def test_06_ann_beyond_transcript_range(self):
        ...
        ac.query('chr3', 187462599)
        gene_info = ac.gene_info
        self.assertEqual(gene_info['id'], 'BCL6')
        self.assertEqual(gene_info['name'], 'BCL6')
        self.assertEqual(gene_info['type'], 'protein_coding')
        self.assertEqual(gene_info['source'], 'RefSeq')
        self.assertEqual(gene_info['strand'], '-')
        rank_info = ac.rank_info
        self.assertEqual(rank_info['trans_id'], 'NM_001706.5')
        self.assertEqual(rank_info['trans_start'], 187439168)
        self.assertEqual(rank_info['trans_end'], 187463256)
        self.assertEqual(rank_info['trans_strand'], '-')
        self.assertEqual(rank_info['exon_total'], 10)
        self.assertEqual(rank_info['cds_min'], 3)
        self.assertEqual(rank_info['cds_max'], 10)
        ...
        self.assertEqual(gene_info['strand'], '-')
        rank_info = ac.rank_info
        self.assertEqual(rank_info['trans_id'], 'NM_001130845.2')
        self.assertEqual(rank_info['trans_start'], 187439168)
        self.assertEqual(rank_info['trans_end'], 187454289)
        self.assertEqual(rank_info['trans_strand'], '-')
        self.assertEqual(rank_info['exon_total'], 10)
        self.assertEqual(rank_info['cds_min'], 3)
        self.assertEqual(rank_info['cds_max'], 10)
    ...
    def test_07_ann_special_cases(self):
        ...
        gene_info = ac.gene_info
        self.assertIsNone(gene_info['id'])
        self.assertEqual(gene_info['name'], 'intergenic(LINC01881,-)')
        self.assertEqual(gene_info['type'], 'intergenic')
        self.assertEqual(gene_info['start'], 243176659)
        self.assertEqual(gene_info['end'], 0)
        self.assertIsNone(gene_info['source'])
        self.assertIsNone(gene_info['strand'])
